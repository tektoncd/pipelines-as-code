...
type Run struct {
	...
	Info    info.Info
	// RepositoryLister is the cached informer-backed lister for Repository resources.
	// Nil in CLI contexts and tests; populated at controller startup.
...
func (r *Run) ListRepositories(ctx context.Context, ns string) ([]apipac.Repository, error) {
	if r.RepositoryLister != nil {
		...
		repos := make([]apipac.Repository, len(listed))
		for i, repo := range listed {
			repos[i] = *repo.DeepCopy()
		}
		return repos, nil
	}
