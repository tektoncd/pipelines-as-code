	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErrSubst != "" {
				...
				if tt.wantSecurityLog {
					found := false
					for _, entry := range observedLogs.All() {
						if entry.Message == "[SECURITY][CRITICAL] Averted GitHub App credential exfiltration attempt before token minting" {
							found = true
							break
						}
					}
					assert.Assert(t, found, "expected critical security log for blocked GitHub App token mint")
				}
				return
			}
			assert.NilError(t, err)
